"""
Comando para verificar o processamento do log 10144.
"""
...
class Command(BaseCommand):
    help = 'Verifica o processamento do log 10144'

    def handle(self, *args, **options):
        # Buscar o log específico
        log = AccessLog.objects.filter(device_log_id=10144).first()
        
        if not log:
            self.stdout.write(self.style.ERROR('Log 10144 não encontrado'))
            return
        
        self.stdout.write(f'=== LOG 10144 ===')
        ...
        # Verificar se o log está sendo processado pelo monitor
        ...
        self.stdout.write(f'Último ID processado: {monitor_status.get("last_processed_id", 0)}')
        self.stdout.write(f'Logs pendentes: {monitor_status.get("pending_logs", 0)}')
