...
@staff_member_required
def interjornada_ativa(request):
    """Página focada apenas em sessões de interjornada ativas."""
    return render(request, 'admin/sessions/interjornada_ativa.html')
...
@staff_member_required
def api_interjornada_ativa(request):
    """API específica para sessões de interjornada ativas."""
    try:
        # Buscar apenas sessões de interjornada (blocked e pending_rest)
        sessoes = EmployeeSession.objects.filter(
            state__in=['blocked', 'pending_rest']
        ).select_related('employee').order_by('-created_at')
        
        # Converter para formato JSON
        sessoes_data = []
        for sessao in sessoes:
            # Calcular tempo decorrido desde o início do bloqueio
            if sessao.block_start:
                tempo_decorrido = timezone.now() - sessao.block_start
                tempo_decorrido_minutos = int(tempo_decorrido.total_seconds() / 60)
            else:
                tempo_decorrido_minutos = 0
            
            # Calcular tempo restante para sair da interjornada
            tempo_restante_liberacao = None
            if sessao.return_time:
                if timezone.now() < sessao.return_time:
                    tempo_restante = sessao.return_time - timezone.now()
                    tempo_restante_liberacao = int(tempo_restante.total_seconds() / 60)
                else:
            ...
            sessoes_data.append({
                ...
                'tempo_decorrido_minutos': tempo_decorrido_minutos,
                'tempo_restante_liberacao': tempo_restante_liberacao,
                'is_in_blacklist': sessao.employee.group.name == 'BLACKLIST_INTERJORNADA' if sessao.employee.group else False,
            })
